...
def code_gen_expr(e):
    ...
    #ADDED TAGS:-----------------------------------------------------------------------------------------------------------------
        elif tag == 'minus': #operands both Int values
                code_gen_expr(e[3]) # first operand object to r1
                asm("ld r2 <- r1[1]", "value from Int") # value from heap
                asm("push r2")
                code_gen_expr(e[4]) # second operand object to r1
                ...
                code_gen_expr([0,"Int","new",(0,"Int")]) # construct a new integer object
                asm("pop r2")
                asm("st r1[1] <- r2", "minus result in returned object") # put the value into the new object 
        elif tag == 'times': #operands both Int values
                code_gen_expr(e[3]) # first operand object to r1
		asm("ld r2 <- r1[1]", "value from Int") # value from heap
		asm("push r2")
		code_gen_expr(e[4]) # second operand object to r1
		...
		asm("ld r1 <- r1[1]", "value from Int") # value from heap
                ...
                asm("li r1 <- 1")
                asm("st r1[1] <- r2", "1 result in returned object")
    elif tag == 'if':
                code_gen_expr(e[3]) # 
    #END OF ADDED TAGS--------------------------------------------------------------------------------------------------------------
	elif tag == 'block':

        ...
        def all_methods(self, name):
                ...
                for i in self.all_methods(self.get_parent(name)):
                        ...
                        if x == None:
                                ...
                                mlist.append(x)
                                overrides[x[0]] = True #ADDED "[0]" TO ORIGINAL CODE. OK???
        ...
        for a in c[3]:
                ...
                if a[0] in ['attribute_init', 'attribute_no_init']:
                        ...
                        if ctab.find_attribute(cname, aname):
                                ...
                                exit()
                        ctab.add_attribute(cname, aname, type, init)#no line numbers
                #Register a method
                if a[0] in ['method']:
                        mname = a[1][1]
                        args = []
        ...
        for f in c[3]: # feature list
            if f[0] == 'method':#process METHODS
                ...
                if not ctab.get_class(f[3][1]):#if method type is not in ctab
                        ...
                        exit()
                stab.add_symbol(f[1][1], f[3][1]) #(methodname, typename)
                #check arg types
                for p in f[2]:
                        if not ctab.get_class(p[1][1]): #if argument types are not in ctab
                                ...
                                stab.add_symbol(p[0][1], p[1][1]) #(argname, typename)
                ...
                if not ctab.get_class(f[2][1]):
                        ...
                        stab.add_symbol(f[1][1], f[2][1]) #(attname, typename)
                check_let_var_types(f[3])
        stab.clear_scope()
                


#Checking for duplicates done in part b?
#####################################################END PART C CODE#########################################################
